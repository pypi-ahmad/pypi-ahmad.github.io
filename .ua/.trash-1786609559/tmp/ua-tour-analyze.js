...
try {
  ...
  const ranked = (counts, field) => [...nodes].map(n => ({ id: n.id, [field]: counts.get(n.id), name: n.name }))
    .sort((a, b) => b[field] - a[field] || a.id.localeCompare(b.id)).slice(0, 20);
  const fanInRanking = ranked(inCount, 'fanIn');
  const fanOutRanking = ranked(outCount, 'fanOut');
  const outValues = [...outCount.values()].sort((a,b)=>a-b);
  const inValues = [...inCount.values()].sort((a,b)=>a-b);
  const highOut = outValues[Math.max(0, Math.floor(outValues.length * .9) - 1)] || 0;
  ...
  const candidates = nodes.map(n => {
    let score = 0; const path = n.filePath || '';
    if (n.type === 'file') {
      if (fileNamePattern.test(n.name || '')) score += 3;
  ...
  }).filter(n => n.score > 0).sort((a,b)=>b.score-a.score || a.id.localeCompare(b.id)).slice(0,5);
  ...
  if (start) {
    ...
    for (let i=0; i<queue.length; i++) {
      const [id, depth] = queue[i]; order.push(id); (byDepth[depth] ||= []).push(id);
      for (const next of adjacency.get(id) || []) if (!(next in depthMap)) { depthMap[next] = depth + 1; queue.push([next, depth + 1]); }
    }
  }
  const details = n => ({id:n.id, name:n.name, type:n.type, summary:n.summary || ''});
  const nonCodeFiles = {
    documentation:nodes.filter(n=>n.type==='document').map(details),
    infrastructure:nodes.filter(n=>['service','pipeline','resource'].includes(n.type)).map(details),
    data:nodes.filter(n=>['table','schema','endpoint'].includes(n.type)).map(details),
    config:nodes.filter(n=>n.type==='config').map(details),
  };
  const pairs = new Set(edges.filter(e => ['imports','calls'].includes(e.type) && byId.has(e.source) && byId.has(e.target)).map(e=>`${e.source}\u0000${e.target}`));
  const clusters = []; const used = new Set();
  for (const key of pairs) {
    const [a,b] = key.split('\u0000'); const normalized = [a,b].sort().join('\u0000');
    if (pairs.has(`${b}\u0000${a}`) && !used.has(normalized)) { used.add(normalized); clusters.push({nodes:[a,b], edgeCount:2}); }
  }
  const index = Object.fromEntries(nodes.map(n=>[n.id,{name:n.name,type:n.type,summary:n.summary || ''}]));
  const result = {scriptCompleted:true,entryPointCandidates:candidates,fanInRanking,fanOutRanking,bfsTraversal:{startNode:start,order,depthMap,byDepth},nonCodeFiles,clusters:clusters.slice(0,10),layers:{count:(graph.layers || []).length,list:(graph.layers || []).map(({id,name,description})=>({id,name,description}))},nodeSummaryIndex:index,totalNodes:nodes.length,totalEdges:edges.length};
