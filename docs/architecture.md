...
Both `/` and `/home` render the homepage. Metadata normalizes `/home` to the canonical root URL. The production build copies `index.html` to `404.html` for GitHub Pages route recovery and to `home/index.html` for direct `/home` requests.
...
| Build | `npm run build` | `build/`, `404.html`, and `home/index.html` exist |
